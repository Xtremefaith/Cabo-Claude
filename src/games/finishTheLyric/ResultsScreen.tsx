...
import { buildFinishLyricLeaderboard, myLatestLyricRound } from './stats';

const MEDALS = ['🥇', '🥈', '🥉'];

export function FinishLyricResultsScreen() {
  ...
  const promptById = useMemo(() => new Map(LYRIC_PROMPTS.map((p) => [p.id, p])), []);
  const standings = useMemo(() => buildFinishLyricLeaderboard(results), [results]);
  const lastRound = useMemo(
    () => myLatestLyricRound(results, myPlayerId ?? ''),
    [results, myPlayerId],
  );
  ...
  return (
    <Screen>
      ...
      <h1 className="mb-1 font-display text-3xl font-extrabold">Finish the Lyric 🏆</h1>
      <p className="mb-4 font-body text-sm text-white/50">
        Most lines nailed across the whole crew.
      </p>
      ...
      <div className="flex flex-1 flex-col gap-3 overflow-y-auto pb-4">
        {standings.length === 0 ? (
          <div className="flex flex-1 flex-col items-center justify-center gap-4 py-10 text-center">
            ...
            <Button onClick={() => navigate('/live/finish-the-lyric')}>
          ...
          <>
            ...
            {/* Your latest round, line by line */}
            {lastRound.length > 0 && (
              <>
                  ...
                  return (
                    <div key={a.promptId} className="glass rounded-2xl p-4">
                      ...
                      >
                        ...
                        {a.correct
                          ? prompt.answer
                          : `You said “${a.pick}” — it was “${prompt.answer}”`}
                      </p>
                    </div>
                  );
                })}
      ...
      <div className="mt-3 flex flex-col gap-2">
        <Button onClick={() => navigate('/live/finish-the-lyric')}>Play again</Button>
