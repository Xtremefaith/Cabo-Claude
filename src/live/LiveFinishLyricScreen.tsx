// Live (synchronous, Kahoot-style) Finish the Lyric. Same room/phase machine as
// the other live games (lobby → question → reveal → final): we show a line of a
// public-domain song and everyone races to pick the line that comes next.
// Trivia scoring — fastest correct pick banks the most (shared speed rule).
...
import { LYRIC_PROMPTS } from '../data/lyricPrompts';
import { DEFAULT_SPICE } from '../data/spice';
import { pickFreshFirst, shuffle, uid } from '../lib/util';
import type { FinishLyricAnswer } from '../types';
...
/** Build a fresh deck at/under the group's spice, preferring unplayed lyrics. */
function buildLyricDeck(maxSpice: number): LiveDeckCard[] {
  const seen = seenLyricIds();
  const pool = LYRIC_PROMPTS.filter((p) => p.spice <= maxSpice);
...
export function LiveFinishLyricScreen() {
  ...
  if (!isCloud()) {
    return (
      <LiveFallback
        ...
        body="Finish the Lyric is a live, play-together game. Connect the group to Supabase to host a room from one phone and race the crew to the next line."
        onHome={() => navigate('/')}
      />
    );
  }
  if (!me) {
    return (
      <LiveFallback
        ...
        body="Finish the Lyric is a group game — join your crew and create your player to play."
        onHome={() => navigate('/')}
      />
    );
  }
...
function StartView({ onStart }: { onStart: () => void }) {
  ...
  const start = async () => {
    const deck = buildLyricDeck(group?.settings?.spice ?? DEFAULT_SPICE);
  ...
  return (
    <Screen>
      <div className="flex flex-1 flex-col items-center justify-center gap-6 text-center">
        ...
        <div>
          <h1 className="font-display text-4xl font-extrabold">Finish the Lyric — Live</h1>
          <p className="mt-2 max-w-xs font-body text-white/55">
            Host a room from this phone. Everyone sees the same line at once — fastest to pick the
            line that comes next scores the most.
          </p>
        </div>
  ...
  return (
    <Screen>
      ...
      <p className="pb-1 pt-2 text-center font-body text-xs font-bold uppercase tracking-widest text-white/40">
        {myAnswer ? 'Locked in — waiting for the crew' : 'Tap the next line — fastest scores most'}
      </p>
      {isHost && (
    ...
    <Screen>
      ...
      <div className="py-4 text-center">
        ...
        <p className="mt-1 font-body text-sm font-bold text-white/50">
          The next line is “{card.answer}”{card.hint ? ` — ${card.hint}` : ''}
        </p>
      </div>
      ...
      <div className="mt-3 flex flex-col gap-2">
        {isHost ? (
          isLast ? (
            ...
            <Button onClick={() => void nextQuestion()}>Next line →</Button>
          )
        ) : (
...
function FinalView({
  ...
  // the Finish the Lyric leaderboard / profiles keep working.
  useEffect(() => {
    ...
    const lyricAnswers: FinishLyricAnswer[] = mine.map((a) => ({
    ...
    addResult({
      ...
      playerId: myId,
      playedAt: Date.now(),
      data: { answers: lyricAnswers },
    });
    localStorage.setItem(flag, '1');
  }, [session.id, answers, myId]);

  const startNew = async () => {
    // A fresh room that prefers lyrics the group hasn't played yet.
    const fresh = buildLyricDeck(group?.settings?.spice ?? DEFAULT_SPICE);
  ...
  return (
    <Screen>
      <div className="py-6 text-center">
        ...
        <p className="font-body text-sm text-white/50">Finish the Lyric — Live</p>
      </div>
